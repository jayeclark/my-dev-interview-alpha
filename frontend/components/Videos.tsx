    ...
    <>
      {allRecords.filter((v: any) => !v.attributes.title || v.attributes.title?.includes(filterBy) || v.attributes.question.data.attributes.question.includes(filterBy)).map((v: any, i: number) => (
        <div 
          ...
          style={{ backgroundColor: activeRecords.includes(v.id) ? "rgba(0,255,0,0.2)" : "", cursor: 'pointer', padding: "8px", display: 'flex', alignItems: 'center', width: '100%', borderTop: i > 0 ? '1px solid #efefef' : 'none' }} 
